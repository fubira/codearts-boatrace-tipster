...
seed_stability_check.py --tune-log PATH --top-n N --seeds 42,100,200,300,400 \
                        --from DATE --to DATE [--gap12-th 0.04] [--ev-threshold -0.25]
                        # tune の上位 N 個を K seed で再学習 → OOS 安定性検証 (winner's curse 検出)
                        # ディスクに model を残さず in-memory で完結 (~30s/trial-seed)
                        # ※ 異なる tune log の trial を cross-compare する時は --ev-threshold 必須。
...
- **EV 閾値は tune 時 ev=0 / 運用・Phase 2 評価時 ev=-0.25 と分離**（2026-04-18 確定）: 公式 `model_prob × odds × 0.75 − 1` は控除率を二重カウントするので EV=-0.25 が「真の期待値 0%」の分岐点 (`knowledge_ev_formula_semantics.md`)。**運用**: ev=-0.25 を default (p2_v3 で P/L peak EV=-0.22 / ROI peak EV=-0.15 / T-1 drift peak EV=-0.25、peak 設定して様子を見る方針。人気サイド買いすぎで損益悪化が見えたら再検討)。**tune**: ev=0 (または ev 不指定で探索) で実行。ev=-0.25 を tune 時に固定すると多購入型 HP が top に来て precision 型 (p2_v3 のような少購入・高精度) が埋もれる (journal [[2026-04-18_1217_hit-pct-adoption-reversal]])。**Phase 2 評価**: 実運用条件で robustness を見るので `seed_stability_check --ev-threshold -0.25` を明示
...
- **Tune は 2 phase 構成**: Phase 1 = WF-CV × growth objective (cheap search、500 trials ~3h)、Phase 2 = **Kelly 上位 N を 5 seed で再評価** し `stability_score = mean(per-seed full-OOS P/L) − std(per-seed full-OOS P/L)` で最終 ranking。Phase 1 は計算速度重視、Phase 2 は seed 分散で Phase 1 noise を filter (p2_v3 が +1.09σ seed-lucky であった経緯で必須)。暦月 split は diagnostic 表示のみ、ranking には per-seed full-period sum を使用 (partial month を含む)。**Phase 3 は立てない** — Phase 2 per-seed row が 5-seed full-OOS 評価そのものであり、追加 stage は情報量ゼロで時間爆発する失敗 pattern。Phase 2 は `server-tune.sh --trials N` でサーバ連続実行、`--phase2` 省略時 `max(15, trials/15)` で auto-scale
...
- **Cross-tune ranking 比較は ev_threshold を揃える**: 異なる tune log の trial を同じ Phase 2 ranking で比較する際、tune log ごとに ev 値が違うと ranking が歪む (2026-04-18 発見、journal `2026-04-18_1028_ev-inheritance-bug-and-pipeline-validation.md`)。必ず `seed_stability_check --ev-threshold <production ev>` を明示する。saved dev model の meta の ev_threshold は tune log の fix_thresholds から継承されるが、これが current production ev と一致するとは限らない
