...
- **Optuna は 2 フェーズ探索**: WF-CV はハイパラと閾値を同時に最適化すると、fold 再学習によりモデルが閾値レジームに適応し、固定モデル OOS で乖離する。Phase 1: `--fix-thresholds` で閾値固定、ハイパラのみ探索（WF-CV）。Phase 2: `--threshold-sweep` で固定モデルに対し b1×ev グリッドサーチ（OOS）
