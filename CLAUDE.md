...
analyze_model.py --from DATE --to DATE [--model-dir DIR]
                 [--split-by none|quarter|month] [--show-importance] [--json]
...
2. Top-1 予測が1号艇であること
3. top3_concentration（`(P_rank2+P_rank3)/(1-P_rank1)`）≥ 閾値 → top3 が下位から分離しているレースを選ぶ
4. gap23（2位-3位のスコア差）≥ 閾値 → 2着予測に自信があるレースを選ぶ
5. 各券の 3連単オッズから `EV = model_prob / market_prob × 0.75 - 1 ≥ 閾値`
