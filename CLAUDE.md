...
tune_trifecta.py --trials N [--seed 42] [--n-folds 4] [--fold-months 2] [--warm-start]
                 [--relevance SCHEME] [--objective sharpe|profit]
...
| 6艇ランキング | 着順予測 | LGBMRanker LambdaRank (28特徴量) | 1着予測（非1号艇） |
...
scrape-daemon が停止しても runner 単体で動作する（Phase 1）。両方動作時の重複スクレイプは DB の upsert で吸収される。

### runner の状態遷移

```
[待機] → 締切7分前 → [展示取得] → 締切5分前 → [オッズ+予測] → 締切1分前 → [ドリフト外挿+購入判断] → 締切5分超過 → [スキップ]
                                                                                                    → 締切12分後 → [結果取得] → [完了]
```

- 起動時: `setupDay()` で会場 discover → スクレイプ → stats snapshot 構築
- ポーリング: 30秒間隔
- 途中起動: 締切5分超過の未処理レースは自動スキップ（Auto-skip ログ出力）
