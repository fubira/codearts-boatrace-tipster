...
- `predict_trifecta.py --date DATE [--snapshot PATH]`: 3連単予測、JSON 出力
- `backtest_trifecta.py --from DATE --to DATE`: 保存済みモデルで OOS 期間評価
- `backtest_trifecta.py --wfcv`: WF-CV（ハイパラ探索の検証専用。本番モデル評価には使わない）
- `tune_trifecta.py`: Optuna ハイパラ探索（WF-CV）。結果は model_meta.json に自動保存
- `train_ranking.py --save --model-meta DIR`: model_meta のハイパラで本番モデル学習・保存
...
./scripts/server-tune.sh --model trifecta --trials 100  # Optuna 実行（3連単戦略）
