...
tune_p2.py --trials N [--seed 42] [--n-folds 4] [--fold-months 2]
           [--relevance podium|linear|top_heavy|search] [--objective hit_pct|growth|kelly]
           [--fix-thresholds "gap23=0.13,top3_conc=0.6,gap12=0.04,ev=-0.25"]
           [--from-model models/<active>[,models/x]] [--narrow]
           [--n-jobs N] [--num-threads N]
           ※ --objective default は hit_pct (+ volume floor)。empirical ρ=+0.648 with
             106-day OOS (2026-04-18 calibration、n=45)。growth/kelly は legacy
...
- **Optuna 目的関数は hit_pct + volume floor を使う** (2026-04-18 確定): `objective = hit_pct if total_races >= PHASE2_MIN_RACES (500) else -1e9`。2026-04-18 calibration (45 HP × 106 日 OOS) で測定した Spearman 相関は hit_pct +0.648 / races +0.631 / growth −0.13 / kelly −0.50 / ROI stability −0.5。**empirical に hit_pct が最強**。growth / kelly / ROI stability は全部 OOS 予測力で劣る (負相関含む) ので使わない。legacy として `--objective growth|kelly` は残すが archive 比較用途のみ。今後の新 metric 提案は必ず calibration dataset で ρ 測定してから採用
...
- **EV 閾値の基本線は -0.25**（2026-04-17 採用）: 公式 `model_prob × odds × 0.75 − 1` は控除率を二重カウントするので EV=-0.25 が「真の期待値 0%」の分岐点。am_476 実測では P/L peak EV=-0.22 / ROI peak EV=-0.15 / T-1 drift peak EV=-0.25 で、**peak 設定して様子を見る方針**。人気サイド買いすぎで損益悪化が見えたら再検討。新モデルの tune / 運用でも `ev=-0.25` を default とする（`knowledge_ev_formula_semantics.md` 参照）
...
- **Tune は 2 phase 構成** (2026-04-18 確定): Phase 1 = WF-CV × hit_pct objective + volume floor (cheap search、500 trials ~3h)、Phase 2 = **hit_pct 上位 N を 5 seed で再評価** し `stability_score = mean(per-seed full-OOS P/L) − std(per-seed full-OOS P/L)` で最終 ranking。Phase 1 は計算速度重視、Phase 2 は seed 分散で Phase 1 noise を filter (p2_v3 が +1.09σ seed-lucky であった経緯で必須)。暦月 split は diagnostic 表示のみ、ranking には per-seed full-period sum を使用 (partial month を含む)。**Phase 3 は立てない** — Phase 2 per-seed row が 5-seed full-OOS 評価そのものであり、追加 stage は情報量ゼロで時間爆発する失敗 pattern (journal `2026-04-18_0928_metric-oos-correlation-calibration.md`)。Phase 2 は `server-tune.sh --trials N` でサーバ連続実行、`--phase2` 省略時 `max(15, trials/15)` で auto-scale
