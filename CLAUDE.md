...
tune_p2.py --trials N [--seed 42] [--n-folds 4] [--fold-months 2]
           [--relevance podium|linear|top_heavy|search] [--objective growth|kelly]
           [--fix-thresholds "gap23=0.13,top3_conc=0.6,gap12=0.04"]
           [--from-model models/<active>[,models/x]] [--narrow]
           [--n-jobs N] [--num-threads N]
           ※ --objective default は growth。kelly は legacy
           ※ --relevance default は podium。"search" で Optuna 探索 (ablation 時のみ)
           ※ tune 時は ev 閾値を fix しない (= 探索) or ev=0 を指定。運用時 ev=-0.25 と
             揃えると precision 型 HP (p2_v3 のような) が多購入型に埋もれる
             ([[2026-04-18_1217_hit-pct-adoption-reversal]])
...
seed_stability_check.py --tune-log PATH --top-n N --seeds 42,100,200,300,400 \
                        ...
                        [--ev-threshold -0.25 | --ev-sweep "0.0,-0.05,...,-0.35"]
                        # tune の上位 N 個を K seed で再学習 → OOS 安定性検証 (winner's curse 検出)
                        # ディスクに model を残さず in-memory で完結 (~30s/trial-seed)
                        # --ev-sweep 指定時は各 trial × seed で複数 ev を評価し peak ev を特定、
                        #   peak ev での stability_score (mean-std) で最終 ranking
                        # --ev-threshold と --ev-sweep は排他。--ev-sweep は server-tune.sh で
                        #   Phase 2 default ("0.0,-0.05,-0.10,-0.15,-0.20,-0.25,-0.30,-0.35") として注入
                        # cross-tune 比較は --ev-threshold / --ev-sweep いずれかを明示統一必須
                        #   (tune log ごとに ev 値が違うと ranking が歪む。journal 2026-04-18_1028)
...
# Phase 2 (Kelly top-N × 5 seed × EV sweep、Phase 1 完了後にサーバで連続実行)
--phase2 N                       # Kelly 上位 N を評価 (default: max(15, trials/15))
                                 # EV sweep (0.0〜-0.35 step 0.05) で各 trial の peak ev を
                                 # 特定、peak ev での stability_score で最終 ranking
...
- **rate 系特徴量の扱い (2026-04-17 確定)**: **global rate** (national_win_rate、motor_top3_rate 等) は市場オッズに織り込み済みで追加情報なし、採用しない。**conditional rate** (racer_course_*、stadium_course_* 等) は条件付き情報で載荷量あり、削除しない。**21feat + podium relevance が P2 の最適構成** (top_heavy 探索でも改善せず、`[[knowledge_public_rate_features_no_edge]]`)
- **Optuna 目的関数は growth を default とする**: `--objective growth` (daily log P/L)。kelly は legacy。2026-04-18 に hit_pct (ρ=+0.648 calibration) を採用したが、calibration dataset (bh/bi/bf/bg) が ev=-0.25 探索の HP 群のみで p2_v3 (ev=0 探索、hit 7-9%) を含まない bias があり、hit_pct objective は p2_v3 を構造的に排除することが判明し撤回 (journal [[2026-04-18_1217_hit-pct-adoption-reversal]])。**新 metric 採用時は必ず「既存 production model が新 metric の top に入るか」sanity check を行う**。dataset に含まれない class を一般結論から漏らすと再発見できない探索環境になる。rank correlation (ρ>0) は "順位予測力あり" であって "max 化しても good HP に到達" を保証しない
...
- **EV 閾値は tune 時 ev=0 / 運用は HP ごとの peak ev と分離**（2026-04-18 確定）: 公式 `model_prob × odds × 0.75 − 1` は控除率を二重カウントするので EV=-0.25 が「真の期待値 0%」の分岐点 (`knowledge_ev_formula_semantics.md`)。**tune 時**: `--fix-thresholds "...,ev=0.0"` で固定。ev=-0.25 を tune 時に固定すると多購入型 HP が top に来て precision 型 (p2_v3 のような少購入・高精度) が埋もれる (journal [[2026-04-18_1217_hit-pct-adoption-reversal]])。**Phase 2 評価**: `seed_stability_check --ev-sweep "0.0,-0.05,...,-0.35"` で各 trial × seed × ev grid を evaluate し、HP ごとの peak ev を特定 (max stability_score、tie-break は高 ev=保守側)。**運用**: 各 model の peak ev を model_meta.json の strategy.ev_threshold に手動更新 (p2_v3=-0.25、bk_120=-0.10 等)。peak ev は HP ごとに異なる (-0.10 〜 -0.25 で分布)
...
- **訓練経路は training.train_p2_ranker に統合** (2026-04-19): train_ranking / train_dev_model / seed_stability_check は共通 API `boatrace_tipster_ml.training.train_p2_ranker` 経由で学習する。val split (end_date - val_months 固定、default 2)、early_stopping_rounds=None、feature_means 計算を統一。**同じ HP + DB で 3 script から bytewise 一致する model が生成される** (p2_v3 で検証済み、md5 `f242d47b88e9e687ebc77504192f69ec`)。訓練関連の振る舞いを変える時は必ず training.py を通す
- **Tune は 2 phase 構成**: Phase 1 = WF-CV × growth objective + ev=0 固定 (200 trials 実測 ~3-6h、CPU 競合あり)、Phase 2 = **Kelly 上位 N を 5 seed × EV sweep で再評価** し、各 HP の peak ev での `stability_score = mean(per-seed full-OOS P/L) − std(per-seed full-OOS P/L)` で最終 ranking。Phase 1 は計算速度重視、Phase 2 は seed 分散で Phase 1 noise を filter + peak ev で HP ごとの真の性能を測る。暦月 split は diagnostic 表示のみ、ranking には per-seed full-period sum を使用。**Phase 3 は立てない** — Phase 2 per-seed row が 5-seed full-OOS 評価そのもの。Phase 2 は `server-tune.sh --trials N` でサーバ連続実行、`--phase2` 省略時 `max(15, trials/15)` で auto-scale、EV sweep は default `"0.0,-0.05,...,-0.35"` (0.05 step 8 点)
- **p2_v3 stability baseline (regression detection 基準、2026-04-18 確定)**: 5-seed × full OOS (2026-01-01〜) ev=-0.25 で **stability_score +31,915 / mean +37,920 / std 6,005 / per-seed range 32,020〜46,920**。p2_v3 劣化検出の基準値。新 tune の Phase 2 winner がこれを超えない限り replacement にしない。測定再現手順: `uv run python -m scripts.seed_stability_check --tune-log logs/tune/2026-04-15_1037_server-tune.log --trials 476 --ev-threshold -0.25 --from 2026-01-01 --to <today>`
- **Cross-tune ranking 比較は ev 条件を揃える**: 異なる tune log の trial を同じ Phase 2 ranking で比較する際、ev 条件 (固定 ev or sweep range) が違うと ranking が歪む (2026-04-18 発見、journal `2026-04-18_1028_ev-inheritance-bug-and-pipeline-validation.md`)。`seed_stability_check --ev-threshold <ev>` か `--ev-sweep <list>` を明示統一。saved dev model の meta の ev_threshold は tune log の fix_thresholds から継承されるが、これが current production ev と一致するとは限らない。Phase 2 で peak ev 特定後は model_meta を peak ev で手動更新
