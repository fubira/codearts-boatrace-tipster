...
- **Tune は 2 phase 構成**: Phase 1 = WF-CV (1 seed, growth objective, ~50 trial)、Phase 2 = Kelly 上位 N を 5 seed で再評価し `stability_score = mean - std` で ranking。Phase 1 の growth/P/L だけで selection するな。**4-12 tune の #266 は growth 1位 だが OOS 最悪 (+6,618 mean, -2,860 min)、kelly 7位**。Kelly が OOS 性能の良い indicator。Phase 2 自動化: `./scripts/server-tune.sh --fetch --phase2 10` で fetch 後に top 10 by Kelly を 5 seed 検証。詳細は memory `project_tune_phase2_workflow.md`
