...
tune_p2.py --trials N [--seed 42] [--n-folds 4] [--fold-months 2]
           [--relevance SCHEME] [--objective profit|growth|kelly]
           [--fix-thresholds "gap23=0.13,top3_conc=0.6,gap12=0.04,ev=-0.25"]
           [--from-model models/<active>[,models/x]] [--narrow]
           [--n-jobs N] [--num-threads N]
           ※ --objective default は profit (volume+hit_pct floor 付き)。
             growth/kelly は archived run との比較用の legacy オプション
...
- **Optuna 目的関数は profit + dual floor を使う** (2026-04-18 改定): `objective = profit if races>=PHASE2_MIN_RACES (500) AND hit_pct>=7.0 else -1e9`。volume floor で naive exploitation (少量購入で高 odds 当選狙い)、hit_pct floor で favorite-heavy 低 ROI HP を排除。profit は odds-weighted なので ROI floor 不要 (同一 volume で ROI 低い HP は profit も自動的に低い)。**Phase 2 selection も profit 基準に揃える** (旧 hit_pct selection は ev=-0.25 環境で ROI=99%/P/L=-1,980 HP を promote する失敗を起こした)。旧 growth/kelly は legacy オプションとして `--objective growth|kelly` で利用可 (archived run との比較用)。growth は log 圧縮で解釈しづらく、ev 閾値変更で value が ±47% 変動し Phase 2 stab と相関 0/10 だった経緯あり
...
- **Tune は 2 phase 構成** (2026-04-18 改定): Phase 1 = WF-CV (1 seed, **profit objective + dual floor**)、Phase 2 = **profit 上位 N** を 5 seed で再評価し `stability_score = mean - std` で ranking。Phase 1 objective と Phase 2 selection を同一 metric (profit) に揃えることで、Phase 1 value ≡ Phase 2 candidate pool の top-N を常に保証 (旧 growth objective + hit_pct selection では両者が乖離し、赤字 HP が Phase 2 に混入する事案が 2026-04-17 bd run で発生)。Phase 2 は `server-tune.sh --trials N` 実行時にサーバで Phase 1 完了後に自動連続実行される (`--phase2` 省略時は `max(15, trials/15)` で auto-scale)
