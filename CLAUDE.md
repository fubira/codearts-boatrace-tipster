...
- **Optuna 閾値は構造的な値として固定**: `gap12=0.04 / top3_conc=0.60 / gap23=0.13 / ev=0.0` は podium relevance で HP 非依存で P/L 最適。HP 変更時に再探索しない。`--fix-thresholds "gap23=0.13,ev=0.0,top3_conc=0.6,gap12=0.04"` を tune の標準。**例外**: 新 relevance_scheme (top_heavy 等) を試す時は sweep 必須。**定期再確認**: 新 OOS 30 日蓄積後に全 HP 共通で sweep し構造変化がないか確認 (年 1-2 回)
...
- **Tune は 2 phase 構成**: Phase 1 = WF-CV (1 seed, growth objective)、Phase 2 = Kelly 上位 N を 5 seed で再評価し `stability_score = mean - std` で ranking。**Phase 1 の growth/P/L だけで selection しない** (growth は high-variance HP を favor する)。Kelly が OOS 性能の良い indicator。Phase 2 は `server-tune.sh --trials N` 実行時にサーバで Phase 1 完了後に自動連続実行される (`--phase2` 省略時は `max(15, trials/15)` で auto-scale)
- **full OOS データを使い regime-robust metric で選定する**: full OOS の mean P/L は high-variance HP (幸運な window で稼ぐ) を favor するので、**min-based / worst-case-based metric** (例: min(early, late_1, late_2) P/L、全 window P/L > 0) を full data に適用して regime-robustness を直接要求する。特定の window を恒久的に除外しない。sub-window split は diagnostic としてのみ使う
