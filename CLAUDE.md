...
- `src/cli/commands/` - コマンド格納先（scrape, scrape-odds, data, backup）
...
| モデル | 目的 | 手法 | 用途 |
|--------|------|------|------|
| 1号艇二値分類 | 1号艇が勝つか予測 | LGBMClassifier (24特徴量) | 単勝 EV 戦略 |
| 6艇ランキング | 着順予測 | LGBMRanker LambdaRank (27特徴量) | 2連複1点買い EV 戦略 |
...
- `FEATURE_COLS` (feature_config.py): ランキングモデル用27特徴量
- `BOAT1_FEATURE_COLS` (boat1_features.py): 二値分類用24特徴量
...
./scripts/server-tune.sh --trials 100      # Optuna 実行
