...
- **Tune は 2 phase 構成**: Phase 1 = WF-CV (1 seed, growth objective)、Phase 2 candidate 選定 = **ROI stability (`mean(rois) - std(rois)`) 上位 N**、Phase 2 最終 ranking = 5 seed stability_score (`mean(P/L) - std(P/L)`)。Phase 1 選定式と Phase 2 最終式を **同じ `mean - std` 形** に揃えて、候補プールが Phase 2 結果と直線的に相関するように設計。ev=-0.25 regime で Kelly log-ROI 差が 1.0-1.15 付近に圧縮されて discrimination power を失った経緯から、linear ROI ベースに切り替え (2026-04-18)。Phase 2 は `server-tune.sh --trials N` 実行時にサーバで Phase 1 完了後に自動連続実行される (`--phase2` 省略時は `max(15, trials/15)` で auto-scale)
