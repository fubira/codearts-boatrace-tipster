...
run [--dry-run(default)] [--live] [--ev-threshold N] [--bet-cap 30000]
    [--unit-divisor 150] [--bankroll 70000]
...
predict_p2.py --date DATE [--snapshot PATH] [--race-ids ID,...] [--use-snapshots]
              [--model-dir models/p2_v1] [--db-path PATH]
...
promote_model.py [--draft models/draft] [--prod models/p2_v1]
...
models/
  ...
  p2_v1/              ← 本番モデル（runner/predict が読む）
    ranking/
```

- 学習: `train_ranking.py` → `models/draft/` に保存
- 探索: `tune_p2.py` → `models/tune_result/` に保存
- 昇格: `promote_model.py` で draft → p2_v1 にコピー（確認プロンプト付き）
- 推論: `predict_p2.py` → `models/p2_v1/` を読む
...
**フィルタ**:
1. Top-1 予測が1号艇であること
2. top3_concentration（`(P_rank2+P_rank3)/(1-P_rank1)`）≥ 閾値 → top3 が下位から分離しているレースを選ぶ
3. gap23（2位-3位のスコア差）≥ 閾値 → 2着予測に自信があるレースを選ぶ
4. 各券の 3連単オッズから `EV = model_prob / market_prob × 0.75 - 1 ≥ 閾値`

**買い方**: P2 = 1-(rank2,rank3)-(rank2,rank3)。最大2点、EV- の券はスキップ。
**ベットサイジング**: `unit = bankroll / divisor`（default: 150）、cap 3万円。`unit × 券数` が1レースの投資額
**EV 判定**: 単勝オッズは使わない（プール薄い）。**各券の3連単オッズ**から直接市場確率を計算
**探索**: `tune_p2.py` で gap23・top3_conc・EV 閾値・ハイパラを同時最適化（growth 目的関数）
...
- モデルは `ml/models/p2_v1/ranking/` に保存
...
- **Optuna は閾値固定→自由探索の反復**: WF-CV はハイパラと閾値を同時に最適化すると、fold 再学習によりモデルが閾値レジームに適応し、固定モデル OOS で乖離する。まず `--fix-thresholds` でハイパラのみ探索 → OOS で閾値を確認 → 必要なら閾値を更新して再探索
