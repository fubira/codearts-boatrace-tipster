...
- `build_features_df()`: 全列を保持した DataFrame を返す（二値分類用）
- `build_features()`: FEATURE_COLS に絞った (X, y, meta) を返す（ランキング用）
...
- `FEATURE_COLS` (feature_config.py): ランキングモデル用25特徴量
- `BOAT1_FEATURE_COLS` (boat1_features.py): 二値分類用28特徴量
- 特徴量の順序は model 互換性のため変更禁止（末尾に追加のみ）

### モデルの保存と推論

- `train_boat1_binary.py --save`: 学習済みモデルを `ml/models/boat1/` に保存
- `predict_boat1.py --date DATE`: 保存済みモデルで指定日を推論、JSON 出力
- `backtest_boat1.py --from DATE --to DATE`: 期間バックテスト + Kelly シミュレーション
...
```
[待機] → 締切30分前 → [展示取得] → 締切7分前 → [オッズ取得+予測] → 締切10分後 → [結果取得] → [完了]
```

- 起動時: 当日の会場 discover → 全レースの出走表スクレイプ → スケジュール構築
- ポーリング: 60秒間隔で各レースの状態を確認し、必要なアクションを実行
- 予測: Python subprocess で `predict_boat1.py` を呼び出し、EV 判定
