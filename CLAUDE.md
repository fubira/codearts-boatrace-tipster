...
- **Optuna 閾値の扱い (2026-04-15 refinement)**: 閾値は **構造的な値** として扱う。`gap12=0.04 / top3_conc=0.60 / gap23=0.13 / ev=0.0` は podium relevance で p2_v2 と p2_v3 (HP が大きく違う) で同じく P/L 最適だった。HP 変更時に再探索しない。`--fix-thresholds "gap23=0.13,ev=0.0,top3_conc=0.6,gap12=0.04"` を tune の標準。**例外**: 新 relevance_scheme (top_heavy 等) を試す時は sweep 必須。**定期再確認**: 新 OOS 30 日蓄積後に全 HP 共通で sweep し構造変化がないか確認 (年 1-2 回)。詳細は memory `feedback_filter_thresholds_structural.md` / `project_threshold_model_coupling.md` (refinement あり)
...
- **新 HP 比較は late OOS (2026-02-13 以降) 単独で実施する**: 2026-04-15 に判明した通り、OOS 期間 2026-01-01〜04-14 は early (Jan 1〜Feb 12、hit 14%) と late (Feb 13〜、hit 8.5%) の heterogeneous な加重平均。early 43 日 (全体の 39%) に 51% の wins / 57% の P/L が集中しており、**p2_v2 の "hit 10.8% / ROI 172%" は early regime bonus を含む**。新 HP や新戦略の比較時は late 単独の数字で評価しないと early-regime 寄生な HP を誤って選んでしまう。詳細は memory `project_regime_shift_early_2026.md`
- **hit% ボーダーは late 単独で 9% 以上**: 旧基準 "full OOS hit% 10%" は retire。500 trial tune の全 trial で full WF-CV hit% 10% 到達はゼロ件、late 単独での ceiling は 9% 台。daily_p2_summary で late 単独 split を確認し、9% を満たさない HP は promote 候補から除外。詳細は memory `feedback_hit_rate_10pct_boundary.md` (改訂版)
