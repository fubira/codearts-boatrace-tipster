...
def train_boat1_2stage(
    ...
    *,
    ...
    """Train a two-stage boat1 model.

    Stage 1: Market-visible features → stage1_score (market's view approximation).
    Stage 2: stage1_score + non-market features + BOATCAST → final b1_prob.

    Returns (stage1_model, stage2_model, metrics).
    """
    from .boat1_features import STAGE1_COLS, STAGE2_COLS

    # Split train/val
    if val_mask is not None:
        train_mask = ~val_mask
        X_train_s1, y_train = X_b1.loc[train_mask, stage1_cols], y[train_mask]
        X_val_s1, y_val = X_b1.loc[val_mask, stage1_cols], y[val_mask]
    else:
        X_train_s1, y_train = X_b1[stage1_cols], y
        X_val_s1, y_val = None, None

    # --- Stage 1: Train on market features ---
    stage1_model, s1_metrics = train_boat1_model(
        X_train_s1, y_train, X_val_s1, y_val,
        n_estimators=n_estimators,
        learning_rate=learning_rate,
        extra_params=extra_params,
    )

    # Generate stage1_score for ALL data (train+val)
    stage1_score = stage1_model.predict_proba(X_b1[stage1_cols])[:, 1]
    X_b1 = X_b1.copy()
    X_b1["stage1_score"] = stage1_score

    # --- Stage 2: Train on non-market + BOATCAST + stage1_score ---
    available_s2 = [c for c in stage2_cols if c in X_b1.columns]
    if val_mask is not None:
        X_train_s2 = X_b1.loc[train_mask, available_s2]
        X_val_s2 = X_b1.loc[val_mask, available_s2]
    else:
        X_train_s2 = X_b1[available_s2]
        X_val_s2, y_val = None, None

    stage2_model, s2_metrics = train_boat1_model(
        X_train_s2, y_train, X_val_s2, y_val,
        n_estimators=n_estimators,
        learning_rate=learning_rate,
        extra_params=extra_params,
    )
    ...
    metrics = {
        "stage1_auc": s1_metrics.get("val_auc"),
        "stage2_auc": s2_metrics.get("val_auc"),
        "stage1_importance": s1_metrics.get("feature_importance", {}),
        "stage2_importance": s2_metrics.get("feature_importance", {}),
        "stage2_features_used": available_s2,
    }

    return stage1_model, stage2_model, metrics


def predict_boat1_2stage(
    ...
    *,
    ...
    """Predict b1_prob using two-stage model.

    Returns array of probabilities (same length as X_b1).
    """
    # Stage 1: predict market score
    stage1_score = stage1_model.predict_proba(X_b1[stage1_cols])[:, 1]
    X_b1 = X_b1.copy()
    X_b1["stage1_score"] = stage1_score

    # Stage 2: predict using non-market + BOATCAST + stage1_score
    available_s2 = [c for c in stage2_cols if c in X_b1.columns]
    return stage2_model.predict_proba(X_b1[available_s2])[:, 1]


def save_boat1_2stage(
    stage1_model: LGBMClassifier,
    stage2_model: LGBMClassifier,
    output_dir: str,
) -> None:
    """Save two-stage model: stage1 in subdir, stage2 as main model.pkl."""
    # Stage 2 = main model
    save_boat1_model(stage2_model, output_dir)
    # Stage 1 = subdir
    stage1_dir = Path(output_dir) / "stage1"
    save_boat1_model(stage1_model, str(stage1_dir))


def load_boat1_2stage(
    model_dir: str,
) -> tuple[LGBMClassifier, LGBMClassifier]:
    """Load two-stage model (stage1 + stage2)."""
    stage2_model = load_boat1_model(model_dir)
    stage1_model = load_boat1_model(str(Path(model_dir) / "stage1"))
